...
exports.SmartReport = functions.https.onRequest(
    async(request, response) => {
        if (request) {
            ...
            let new_text = '\n' + '\n' + '*Who uses the most foul language?*';
            for (var i = 0; i < keyValues_badWords.length; i++) {
                if (keyValues_badWords[i][1] != 0) {
                    new_text = new_text + '\n' + String(i + 1) + '. ' + '<@' + String(keyValues_badWords[i][0]) + '>' + ' has ' + String(keyValues_badWords[i][1]) + ' messages with foul language';
                    if (i == 0) {
                        var bad_sentence = '<@' + String(keyValues_badWords[i][0]) + '>' + ' uses somewhat foul language when communicating.';
                    }
                }
            }
            ...
            new_text = new_text + '\n' + '\n' + '*Who dominates the conversation the most?*'
            for (var i = 0; i < keyValues_numMessages.length; i++) {
                if (keyValues_numMessages[i][1] == 1) {
                    new_text = new_text + '\n' + String(i + 1) + '. ' + '<@' + String(keyValues_numMessages[i][0]) + '>' + ' has spoken ' + String(keyValues_numMessages[i][1]) + ' time';
                    if (i == 0) {
                        var alpha_sentence = '<@' + String(keyValues_numMessages[i][0]) + '>' + ' seems to be dominating the conversation.';
                    }
                } else {
            ...
            new_text = new_text + '\n' + '\n' + '*Who needs to speak up in the conversation?*'
            for (var i = 0; i < keyValues_numMessages.length; i++) {
                if (keyValues_numMessages[i][1] == 1) {
                    new_text = new_text + '\n' + String(i + 1) + '. ' + '<@' + String(keyValues_numMessages[i][0]) + '>' + ' has spoken ' + String(keyValues_numMessages[i][1]) + ' time';
                    if (i == 0) {
                        var speakup_sentence = ' <@' + String(keyValues_numMessages[i][0]) + '>' + ' should be encouraged to speak more.';
                    }
                } else {
                    new_text = new_text + '\n' + String(i + 1) + '. ' + '<@' + String(keyValues_numMessages[i][0]) + '>' + ' has spoken ' + String(keyValues_numMessages[i][1]) + ' times';
                    if (i == 0) {
            ...
            var new_text1 = bad_sentence + alpha_sentence + speakup_sentence
            new_text = new_text1 + new_text
            ...
            axios
                .post("INSERT SLACK WEBHOOK LINK", {
                    text: new_text,
                })
                .then((res) => {})
                .catch((error) => {
